...
func (c *Config) loadAndValidate() error {
	...
	if c.AccountFile != "" {
		contents, _, err := pathorcontents.Read(c.AccountFile)
		if err != nil {
			return fmt.Errorf("Error loading Account File: %s", err)
		}

		// Assume account_file is a JSON string
		if err := parseJSON(&account, contents); err != nil {
			return fmt.Errorf("Error parsing account file '%s': %s", contents, err)
		}
